...
/**
 * POST /api/vaults/withdraw/submit — sign-bind and broadcast a vault withdrawal.
 *
 * Step two of the non-custodial withdraw: the browser signed the prepared tx
 * hash with the user's own wallet; here we bind that signature to the caller's
 * key (resolved server-side) and submit the Soroban transaction via Stellar RPC.
 */
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    return json(502, { error: "Could not submit the withdrawal" });
  }
};
